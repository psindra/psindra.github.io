var active = false;
// const cssCode = ":root { color-scheme: only light};";
const cssCode = ":root { filter: invert(1) hue-rotate(180deg)};";
...
function updateColorSchemeCSS(active) {
    if (active) {
        chrome.tabs.removeCSS(null, { code: cssCode, allFrames: true }),
        chrome.tabs.insertCSS(null, { code: cssCode, allFrames: true });
    } else {
        (chrome.tabs.removeCSS(null, { code: cssCode, allFrames: true }),
        chrome.tabs.removeCSS(null, { code: cssCode, allFrames: true }),
        chrome.tabs.removeCSS(null, { code: cssCode, allFrames: true }));
